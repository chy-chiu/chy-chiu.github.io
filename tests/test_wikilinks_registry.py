...
class TestWikiLinksRegistry(unittest.TestCase):
    def test_null_title_falls_back_to_filename_stem(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            page = index.writing[0]
            self.assertEqual(page.title, "My Obsidian Note")
            self.assertEqual(page.url, "/blog/my-obsidian-note/")
    ...
    def test_wikilink_resolves_by_filename_stem_not_title(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertIn('<a href="/blog/different-title/">My Obsidian Note</a>', processed.html)
            self.assertIn('<a href="/blog/different-title/">Alias</a>', processed.html)
    ...
    def test_wikilink_ignores_heading_and_block_refs(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertIn('<a href="/blog/deep-title/">Deep Note#Section</a>', processed.html)
            self.assertIn('<a href="/blog/deep-title/">Deep Note^block</a>', processed.html)
